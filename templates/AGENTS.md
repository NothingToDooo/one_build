...
把 `raw/` 视为 agent 工作区和不可变原始资料区，把 `实体/`、`概念/`、`对比/`、`问答/`、`总结/` 视为用户会直接阅读的 wiki 页面区。除非用户明确要求，不要删除、覆盖或重写原始资料。

全局 skill 只负责定位这个目录。进入本目录后，以 `raw/AGENTS.md` 和 `raw/SCHEMA.md` 为准。
...
- `实体/`：人物、组织、产品、项目、模型、工具、地点。
- `概念/`：概念、方法、原则、主张、主题。
- `对比/`：对比分析。
- `问答/`：值得保留的复杂问答。
- `总结/`：跨来源、跨主题的综合总结和阶段性综述。
...
3. 原始资料必须保留来源信息；新建 Markdown 或提取 sidecar 时，按 `raw/SCHEMA.md` 写入 raw frontmatter。
4. 计算正文 sha256。sha256 对 frontmatter 之后的正文计算，不包含 frontmatter 自身。
5. 先查 `raw/index.md` 和已有 wiki 页面，确认相关实体、概念、比较和查询是否已存在。
6. 只有资料符合 `raw/SCHEMA.md` 的建页阈值时，才创建新页面；能更新既有页面时不要制造重复页。
...
优先使用 `raw/tools/llmwiki_tool.py`，把可确定的检查和批量修改交给它完成。你负责判断“应该怎么改”，工具只负责验证路径、执行机械改动和输出结构化结果。
...
1. 断开的 `[[wikilinks]]`。
2. 没有入链的孤立页面。
3. 未登记到 `raw/index.md` 的 wiki 页面。
4. `raw/index.md` 中存在但文件缺失的页面。
5. 缺失或无效 frontmatter 的页面。
6. 未登记在 `raw/SCHEMA.md` 的标签。
7. `confidence: low` 页面。
8. 单一来源但没有置信度标记的页面。
...
1. 在 `raw/_archive/` 下保留原始分类路径，例如 `raw/_archive/实体/旧页面.md`。
...
obsidian create path="llmwiki/问答/example.md" content="# Example\n\nContent" open
obsidian append path="llmwiki/问答/example.md" content="\nMore content"
```
